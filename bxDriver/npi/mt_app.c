...
int mtsapi_WriteConfiguration(uint8_t configId, uint8_t len, uint8_t *valTab)
{
    ...
    status = NPISendSynchData(MT_SAPI_WRITE_CFG_REQ, pbuf, len + 2);
...
{
    return NPISendSynchData(MT_SAPI_READ_CFG_REQ, &configId, 1);
}

int mtsapi_GeDeviceInfo(uint8_t Param)
{
    return NPISendSynchData(MT_SAPI_GET_DEV_INFO_REQ, &Param, 1);
}
...
int mtsapi_PermitJoingRequest(uint16_t dstaddr, uint8_t timeout)
{
    ...
    return NPISendSynchData(MT_SAPI_WRITE_CFG_REQ, buf, 3);
}
*/
...
{        
    return NPISendSynchData(MT_SAPI_WRITE_LOGICAL_TYPE, &logicaltype, 1);
}

int mtsapi_StartNwk(uint8_t mode)
{
    return NPISendSynchData(MT_SAPI_START_NWK, &mode, 1);
}
