...
static void   setItem( uint8_t pg, uint16_t ItemDataoffset, eNvHdrEnum stat );
...
static void setItem( uint8_t pg, uint16_t ItemDataoffset, eNvHdrEnum stat )
{
    ...
    if ( stat == eNvXfer ) {
        ...
        writeHalfWord( pg, ItemDataoffset - NV_ITEM_HDR_SIZE + NV_ITEM_HDR_CHK_OFFSET, (uint8_t*)(&(hdr.stat)) );
    }
    else {// if ( stat == eNvZero )
    
        uint16_t sz = ((hdr.len + (NV_WORD_SIZE - 1)) / NV_WORD_SIZE) * NV_WORD_SIZE + NV_ITEM_HDR_SIZE;
        hdr.id = 0;
        writeHalfWord( pg, ItemDataoffset - NV_ITEM_HDR_SIZE, (uint8_t *)(&hdr.id) );
