...
//uint8_t ltlprefixHdrsize(uint8_t *refer);
//uint8_t *ltlPrefixBuildHdr( uint8_t *refer, uint8_t *pDat );
//uint8_t ltlPrefixrequest(uint8_t *pDat, uint16_t buflen);
// 提供网络帧头长度
#define ltlprefixHdrsize(refer) nwkHdrLen()
// 填充网络帧头
#define ltlPrefixBuildHdr(refer, pDat ) nwkBuildHdr(pDat, NWK_FC_DATA , *((uint16_t *)refer), 0)
// 直接发送
